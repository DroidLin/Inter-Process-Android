            ...
            stringBuilder
                ...
                .also { builder ->
                    ...
                    abstractKotlinNonSuspendFunctionDeclaration.forEach { ksFunctionDeclaration ->
                        ...
                        if (extensionReceiver != null) {
                            builder.appendLine("\t\t\t\"${ksFunctionDeclaration.identifier}\" -> this@${classDeclaration.simpleName.asString()}Receiver.rawInstance.run { (functionParameters[0] ${if (extensionReceiver.resolve().isMarkedNullable) "as?" else "as"} ${extensionReceiver.typeOfQualifiedName}).${ksFunctionDeclaration.simpleName.asString()}${ksFunctionDeclaration.parameters.mapIndexed { index, ksValueParameter -> "(functionParameter[${index + 1}] ${if (ksValueParameter.type.resolve().isMarkedNullable) "as?" else "as"} ${ksValueParameter.type.typeOfQualifiedName})" }.joinToString(separator = ", ", prefix = "(", postfix = ")")} }")
                        } else {
                            builder.appendLine("\t\t\t\"${ksFunctionDeclaration.identifier}\" -> this@${classDeclaration.simpleName.asString()}Receiver.rawInstance.${ksFunctionDeclaration.simpleName.asString()}${ksFunctionDeclaration.parameters.mapIndexed { index, ksValueParameter -> "(functionParameters[${index}] ${if (ksValueParameter.type.resolve().isMarkedNullable) "as?" else "as"} ${ksValueParameter.type.typeOfQualifiedName})" }.joinToString(separator = ", ", prefix = "(", postfix = ")")}")
                        }
                    }
                }
